...
int main(int argc, char** argv)
{
	...
	// Get button numbers to collect waypoints and end collection
		ros::param::get("/ASV_CATAM_URDF_ASSEMBLY/collect_button_num", collect_button_num);
		ros::param::get("/ASV_CATAM_URDF_ASSEMBLY/end_button_num", end_button_num);

    //Initiate subscribers
		ros::Subscriber sub_joy = n.subscribe("/joy", 100, joy_CB);
		ros::Subscriber sub_gps = n.subscribe("/ASV_CATAM_URDF_ASSEMBLY/gps/filtered", 100, filtered_gps_CB);
		ROS_INFO("Initiated collect_gps_waypoints node");

	// Initiate publisher to send end of node message
		ros::Publisher pubCollectionNodeEnded = n.advertise<std_msgs::Bool>("/ASV_CATAM_URDF_ASSEMBLY/collection_status",100);

    //Read file path and create/open file
    	ros::param::get("/ASV_CATAM_URDF_ASSEMBLY/coordinates_file", path_local);
		std::string path_abs =  ros::package::getPath("ASV_CATAM_URDF_ASSEMBLY") + path_local;	
		std::ofstream coordFile (path_abs.c_str());
		ROS_INFO("Saving coordinates to: %s", path_abs.c_str());
		
	// Give instructions:
		ros::param::get("/ASV_CATAM_URDF_ASSEMBLY/collect_button_sym", collect_button_sym);
		ros::param::get("/ASV_CATAM_URDF_ASSEMBLY/end_button_sym", end_button_sym);
		std::cout << std::endl;
